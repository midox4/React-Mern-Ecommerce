...
const VerifyEmail = () => {
  ...
  useEffect(()=>{
    const getUser = async() => {
      setLoad(true)
      try{
    const {data} = await Axios.get(`/AUTH/${userId}/VERIFY/${tokens}`)
          console.log(data);
    ...
    <Box sx={{height: '100vh' , display : 'flex', bgcolor: 'black' , alignItems: 'center' , justifyContent: 'center' }}>
        ...
        <Box sx={{ display : 'flex' , flexDirection: 'column'}}>
          <Box  sx={{ display : 'flex' , alignItems: 'center'}}>
            <VerifiedIcon sx={{ color: "green" , marginRight: "20px" , fontSize:'40px'}} />
        ...
        <Box sx={{ display : 'flex', marginTop: "40px" , alignItems: 'center' , justifyContent: 'center'}}>
            <Link to='/login'>
             <Button  variant="outlined" sx={{ color: 'white'}}> Go To Login</Button>
            </Link>
            </Box>
            </Link>
            </Box>
