...
module.exports.LOGIN = async (req, res, next) => {
  try {
    ...
    if (!result.isAccountVerified) {
      return next(createError(423, "We sent to you an email, please verify your email address"));
    }

    const user = await UUSER.findOne({ Email: email }).select("-Password").select("-Email");
